...
class TVShowScanner:
    ...
    def __init__(self, tvshow_path, job=None):
        ...
        try:
            self.tvdbcache_obj = TVShow.objects.get(localpath=self.tvshow_path)
            ...
            try:
                ...
                self.tvdbcache_obj = TVShow.objects.get(id=int(self.tvdbshow_obj['id']))
                logger.debug("Found matching tvdbcache item %s" % self.tvdbcache_obj.id)
            except:
        ...
        self.search_names = self.tvdbcache_obj.get_titles()
    ...
    def _ep_exists_on_ftp(self, season, ep):
        ...
        for ftp_rls in self.existing_server_entries:
    ...
    def __add_new_download(self, season):
        ...
        for rls in self.existing_server_entries:

            ftp_rls_name = os.path.basename(rls)
    ...
    def _try_download_season(self, season, onlyget=None):
        ...
        self.log("Trying to download season %s, frist lets check the ftp" % season)

        ftp_path, got_seasons = self._season_exists_on_server(season)

        if ftp_path:
            self.log("Found existing season on the ftp %s" % ftp_path)
            ...
            self.existing_server_entries.append(ftp_path.strip())
            return True

        #lets check on on the prescan
        if self.__pre_scan == None:
            self.log("Doing a prescan for seasons, results will be cached for performance")
            found_scc = lazyapi.search_torrents(self.search_names)
            ftpmanager.getTVTorrentsSeason('scc-archive', self.search_names, 0)
            found_tl = ftpmanager.getTVTorrentsSeason('tl-packs', self.search_names, 0)
            ...
            for site in self.__pre_scan:
                ...
                for torrent in torrents:
                    ...
                    if season in found_seasons:
                        ...
                        #TODO: Exception checking here and check if null
                        seasonPath = ftpmanager.download_torrent(site, torrent)

                        if seasonPath is not None and seasonPath is not False and seasonPath != '':
                            if len(got_seasons) == 1:
    ...
    def _get_missing_whole_seasons(self, force_seasons=[], fix_missing_seasons=False):
        ...
        self.log("Looking for whole missing seasons")
        missing_seasons = []
            ...
            if cur_season['status'] == self.SEASON_EXISTS or cur_season['status'] == self.SEASON_MISSING:
                ...
                if cur_season['status'] == self.SEASON_EXISTS:
                    ...
                    if cur_season_no in force_seasons:
                        ...
                        self.log("Skipping season %s as we are not meant to fix it" % cur_season_no)
        ...
        if len(missing_seasons) > 0:
            self.log("We are missing the following whole seasons %s") % missing_seasons
        else:
            self.log("Didn't find any whole seasons missing")

        return missing_seasons


    def _get_seasons_in_queue(self):
        self.log("Looking for seasons in the queue already")
        found = []
        ...
        if len(found) > 0:
            self.log("Found seasons in queue %" % found)
        else:
            self.log("No seasons found in queue")
        return found