...
class Tvdb:
    ...
    def __init__(self,
        ...
        if apikey is not None:
            ...
            try:
                ...
                self.config['apikey'] = settings.TVDB_ACCOUNTID # tvdb_api's API key
            except:
                raise tvdb_error("Unable to load API Key from config")
        ...
        self.config['convert_xem'] = convert_xem
        ...
        self.config['base_url_xem'] = "http://thexem.de"

        self.config['xem_all_shows'] = u"%(base_url_xem)s/map/allNames?origin=tvdb" % self.config

        self.config['url_getSeriesFavs'] = u"%s/api/User_Favorites.php?accountid=%s" % (self.config['base_url'], self.config['apikey'])
    ...
    def _getetsrc(self, url, language=None, type="XML", recache=False):
        """Loads a URL using caching, returns an ElementTree of the source
        """
        src = self._loadUrl(url, language=language, recache=recache)
    ...
    def _setItem(self, sid, seas, ep, attrib, value):
        ...
        """
        #log().debug("called setitem %s %s %s %s %s" % (sid, seas, ep, attrib, value))
    ...
    def _getShowData(self, sid, language):
        ...
        #try load ep data from thexem.de
        convert_xem = False
        if self.config['convert_xem'] == True:
            url = "%s/map/all?id=%s&origin=tvdb" % (self.config['base_url_xem'], sid)
            try:
                xem_data = self._getetsrc(url, type="JSON")
                result = xem_data['result']
                if result == "success":
                    #we found the data.. so lets try convert using xem
                    convert_xem = True
            except:
                pass
        ...
        for cur_ep in epsEt.findall("Episode"):
            ...
            if convert_xem:
                #Whats the real ep and season no
                seas_no, ep_no = self.convert_ses_ep_to_scene(xem_data, sid, seas_no, ep_no)
    ...
    def convert_ses_ep_to_scene(self, data, sid, season, ep):
        try:
            ...
            for mapping in data['data']:
                ...
                try:
                    ...
                    tvdb_season = tvdb['season']

                    if tvdb_season == season and tvdb_ep == ep:
                        scene = mapping['scene']

                        scene_ep = scene['episode']
                        scene_season = scene['season']

                        if scene_ep is not None and scene_season is not None:
                            return int(scene_season), int(scene_ep)
    ...
    def get_xem_show_convert(self, sid, season, ep):
        #load the xem show
        url = "%s/map/all?id=%s&origin=tvdb" % (self.config['base_url_xem'], sid)

        try:
            src = self._getetsrc(url, type="JSON")

            result = src['result']

            if result == "success":
                for mapping in src['data']:
                    ...
                    try:
                        ...
                        if scene_season == season and scene_ep == ep:
                            log().debug("Found a matching thexem entry")

                            tvdb = mapping['tvdb']
                            tvdb_ep = tvdb['episode']
                            tvdb_season = tvdb['season']

                            if tvdb_ep is not None and tvdb_season is not None:
                                return tvdb_season, tvdb_ep
    ...
    def get_favs(self):
        """This searches TheTVDB.com for the users favs
        and returns the result list
        """
        log().debug("Searching for tvdb favs")

        favs_et = self._getetsrc(self.config['url_getSeriesFavs'], recache=True)
    ...
    def add_fav(self, id):
        url = u"%s/api/User_Favorites.php?accountid=%s&type=add&seriesid=%s" % (self.config['base_url'], self.config['apikey'], id)
        self._loadUrl(url, recache=True)

        #Add it to the master server fav
        url = u"%s/api/User_Favorites.php?accountid=289F895955772DE3&type=add&seriesid=%s" % (self.config['base_url'], id)
        self._loadUrl(url, recache=True)
        return True


    def del_fav(self, id):
        url = u"%s/api/User_Favorites.php?accountid=%s&type=remove&seriesid=%s" % (self.config['base_url'], self.config['apikey'], id)
        self._loadUrl(url, recache=True)
        return True
