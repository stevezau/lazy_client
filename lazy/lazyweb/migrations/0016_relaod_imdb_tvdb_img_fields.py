...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for imdbobj in orm.Imdbcache.objects.all():
            if imdbobj.posterimg:
                imdbobj.posterimg = str(imdbobj.id) + "-imdb.jpg"
                imdbobj.save()

        for tvdbobj in orm.Tvdbcache.objects.all():
            # You must use a path relative to the settings.MEDIA_ROOT.
            if tvdbobj.posterimg:
                tvdbobj.posterimg = str(tvdbobj.id) + "-tvdb.jpg"
                tvdbobj.save()
