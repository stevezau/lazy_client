...
from lazyweb.forms import Find, FindMissing
from lazyweb.utils.ftpmanager import FTPManager
from lazyweb import utils
from lazyweb.models import DownloadItem, Job
...
class FindListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            results = ftpmgr.search_torrents(search)
...
def download_torrent(items):
    ...
    for item in items:
        try:
            ...
            ftpmgr = FTPManager()
            path = ftpmgr.download_torrent(site, torrent)
...
def update(request, type):
    ...
    if request.method == 'POST':
        ...
        try:
            function = utils.load_button_module("lazyweb.views.other", type)
